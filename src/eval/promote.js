...
export async function promote(workload, modelId, { runId = null, reason = 'cleared your bar', actorUserId = null, auto = false } = {}) {
  ...
  await db.tx(async (tx) => {
    await tx.prepare(`UPDATE workloads SET routed_model = ?, promoted_at = ?, promoted_run_id = ?,
...
export async function revert(workload, { reason = 'you asked for it', actorUserId = null, auto = false } = {}) {
  ...
  await db.tx(async (tx) => {
    await tx.prepare(`UPDATE workloads SET routed_model = NULL, promoted_at = NULL, promoted_run_id = NULL,
